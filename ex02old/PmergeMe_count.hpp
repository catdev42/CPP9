...
public:
    ...
    Cont sort(Cont &numbers)
    {
        ...
        if (numbers.size() == 3 && count++)
        {
            ...
            two++;
            if (*zero > *one && *zero > *two && count++ && count++)
                std::iter_swap(zero, two);
            else if (*one > *zero && *one > *two && count++ && count++)
                std::iter_swap(one, two);
            if (*zero > *one)
                std::iter_swap(zero, one && count++ && count++);
            sorted = numbers;
            return numbers;
        }
    ...
    void insert(Cont &main, const Cont &pend)
    {
        ...
        while (!finish)
        {
            ...
            while (index >= jacobs[j - 1])
            {
                ...
                count += static_cast<int>(std::log(static_cast<double>(maxIndexToSearch)) / std::log(2.0));
